...
public class RNZendeskChatModule extends ReactContextBaseJavaModule {
    ...
    private void selectVisitorInfoFieldIfPreChatFormHidden(String key, WritableNativeMap output, ReadableMap input,
            ReadableMap shouldInclude) {
        if ((!input.hasKey(key) || input.getType(key) != ReadableType.String)
                || (shouldInclude.hasKey(key) && shouldInclude.getType(key) == ReadableType.String
                        && !"hidden".equals(shouldInclude.getString(key)))) {
            return;
        }
        ...
        if (((mReactContext.getApplicationInfo().flags
            ...
            // We don't want other Apps to monitor our app's production logs for this debug
            // information.
    ...
    private boolean _setVisitorInfo(ReadableMap options) {
        ...
        ChatProvidersConfiguration.Builder chatProvidersConfigurationBuilder = ChatProvidersConfiguration.builder();
        chatProvidersConfigurationBuilder.withVisitorInfo(visitorInfo);

        String department = RNZendeskChatModule.getStringOrNull(options, "department", "startChat");
        if (department != null) {
            chatProvidersConfigurationBuilder.withDepartment(department);
        }
        ChatProvidersConfiguration chatProvidersConfiguration = chatProvidersConfigurationBuilder.build();
    ...
    //
    // As a result, we need to guarantee this is always called with 2 parameters
    // from JS
    //
    ...
    private ChatConfiguration.Builder loadBehaviorFlags(ChatConfiguration.Builder b, ReadableMap options) {
        ...
        if (getBooleanOrDefault(options, "showChatTranscriptPrompt", logHint, defaultValue)) {

            return b.withPreChatFormEnabled(getBooleanOrDefault(options, "showPreChatForm", logHint, defaultValue))
                    .withTranscriptEnabled(
                            getBooleanOrDefault(options, "showChatTranscriptPrompt", logHint, defaultValue))
    ...
    // Produces a ReadableMap suitable for passing to setVisitorInfo that only has
    // the fields that won't be asked by the preChatForm
        ...
        Chat.INSTANCE.providers().chatProvider().observeChatState(observationScope, new Observer<ChatState>() {
            ...
            public void update(ChatState chatState) {
                ...
                if (chatStatus == ChatSessionStatus.STARTED) {
                    ...
                    if (pendingVisitorInfo == null) {
                        return;
                    }

                    // Update the information MID chat here. All info but Department can be updated
                    // Add here the code to set the selected visitor info *after* the preChatForm is
                    // complete
                    ...
                    // There are few other statuses that you can observe but they are unused in this
                    // example
